...
with open('0_500_prof_dim_samples.pkl', 'rb') as f:
    pickle.dump(f)
'''
    ...
    for j in lambd_list:
        ...
        for i in range(len(gen_samples)):
            ...
            if j == 0:
                ...
                #G = sinkhorn_stabilized(data, halton_points, j)
                row_sum = G.sum(axis = 1)
                scaled_G = G / row_sum[:, np.newaxis]
            ...
            if i % 100 ==0: print('sRE=  %0.6f' %(sRE[i]))
        results[str(j) +'sRE'] = sRE
        #results[str(j) + 'sRmmd'] = sRmmd    
    stat_dict[key] = results

with open('500_prof_dim_check.pkl','wb') as f:
